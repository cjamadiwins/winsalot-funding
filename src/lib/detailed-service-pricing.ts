...
function paragraphHtml(text: string): string {
  return `<p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#374151;">${escapeHtml(text).replace(/\n/g, "<br>")}</p>`;
}
...
export function buildDetailedServicePricingHtml(input: {
  ...
  const parts = template.paragraphs.map((paragraph) => {
    if (paragraph === "What Our Lead Generation Service Includes") {
      return `<h2 style="margin:22px 0 10px;font-size:17px;color:#1E3A5F;">${paragraph}</h2><ul style="margin:0 0 18px;padding-left:22px;color:#374151;line-height:1.7;">${template.bullets.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>`;
    }
  ...
  parts.splice(
    insertionIndex,
    0,
    `<div style="margin:22px 0;text-align:center;"><a href="${safeUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:13px 24px;border-radius:8px;background:#2563EB;color:#FFFFFF;font-weight:700;text-decoration:none;">Continue with Winsalot Corp.</a></div>`
  );

  return `<div style="max-width:640px;margin:0 auto;font-family:Arial,Helvetica,sans-serif;color:#374151;"><div style="padding:18px 22px;background:#F5F9FF;border-bottom:1px solid #D9E6F2;border-radius:12px 12px 0 0;"><div style="font-size:20px;font-weight:700;color:#1E3A5F;">Winsalot Corp.</div><div style="margin-top:3px;font-size:13px;color:#52677D;">Empowering Businesses, One Solution at a Time.</div></div><div style="padding:24px 22px;border:1px solid #e2e8f0;border-top:0;border-radius:0 0 12px 12px;background:#FFFFFF;">${parts.join("\n")}</div></div>`;
}
