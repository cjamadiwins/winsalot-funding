...
// consultation-guide follow-up), parametrized only by the partner's own
// name and their actual saved revenue-share/commission-share percentages
// (never hardcoded), so the numbers a future referral partner with a
// different split sees are always their own real agreed terms, not
// Tony's specific 40/60.
...
// matching the brief's Tony-specific numbers) so the worked examples in
// the email always match what's actually configured on this partner's
// profile.
export function buildPartnerOverviewEmailDraft(
  partner: Pick<SubcontractorProfileRow, "full_name" | "lead_gen_revenue_share_percent" | "lending_commission_share_percent">
