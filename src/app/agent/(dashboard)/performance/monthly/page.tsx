...
import { crmWeekStartOf, crmDateKey } from "@/lib/crm-performance";
...
//    exactly the way the existing weekly page (/agent/performance)
//    already does.
//  - the one-time freeze-completed-periods sync needs the service-role
//    admin client (agents have no insert/update grant on
//    crm_agent_weekly_performance, only select-own - migration 0052),
...
export default async function AgentCrmMonthlyPerformancePage() {
  ...
  const now = new Date();
  await syncCrmWeeklyPerformanceHistory(admin, [{ id: crmUser.id, full_name: crmUser.full_name, email: crmUser.email }], records, now);
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-text-muted)]">
            Your opportunity leads added, consultations booked, and emails delivered for the selected month, against goals
            based on the weekly (Monday-Friday) targets that fall within it. Previous months stay available after the month
            changes.
          </p>
        </div>
