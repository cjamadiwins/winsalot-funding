...
export default async function AgentCallListSegmentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!segment) {
    return (
      <div className="mx-auto mt-16 max-w-md rounded-2xl border border-[var(--color-border)] bg-[var(--color-input-bg)] p-8 text-center">
        ...
        <Link href="/agent/call-list-segments" className="mt-5 inline-block rounded-full bg-[var(--color-accent)] px-4 py-2 text-sm font-semibold text-white">
  ...
  return (
    <div>
      <Link href="/agent/call-list-segments" className="text-[13px] font-medium text-[var(--color-accent)]">
        ← Back to My Call Lists
      </Link>
