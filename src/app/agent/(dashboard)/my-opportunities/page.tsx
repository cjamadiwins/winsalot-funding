...
export default async function AgentMyOpportunitiesPage({
  ...
  const [{ data: scores }, { data: activities }, { data: appointments }] = await Promise.all([
  ...
  return (
    <div>
      ...
      <MyOpportunitiesClient rows={rows} initialView={view === "board" ? "board" : "list"} onAddNote={addBoardOpportunityNoteAction} />
    </div>
  );
}
