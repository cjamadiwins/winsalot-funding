import { requestLeadgenPasswordResetAction } from "./actions";

export default async function LeadgenForgotPasswordPage({
  searchParams,
}: {
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--color-bg)] px-4">
      <div className="w-full max-w-sm rounded-2xl border border-[var(--color-border)] bg-[var(--color-input-bg)] p-8 shadow-sm">
        ...
        {sent ? (
          ...
          <form action={requestLeadgenPasswordResetAction} className="mt-6 space-y-4">
            <div>
        ...
        <a href="/leadgen/login" className="mt-6 block text-center text-sm font-medium text-[var(--color-accent)]">
          Back to sign in
        </a>
      </div>
    </div>
