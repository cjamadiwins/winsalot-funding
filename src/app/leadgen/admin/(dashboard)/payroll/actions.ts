...
// the page and go to Agent Management. Mirrors
// updatePayrollAgentCurrencyAction in the Growth CRM's equivalent
// actions.ts - a single-field update scoped to exactly one agent
// (leadgen_users.id = agentId), never touching full_name/role/active, so
// it can't accidentally overwrite those from stale form state carried on
// the payroll page. Currency is purely a display/formatting concept (see
// formatCurrency's header comment in src/lib/payroll.ts) - this never
// changes a wage amount or recalculates any existing payroll record.
export async function updateLeadgenPayrollAgentCurrencyAction(agentId: string, currency: string): Promise<ActionResult> {
  await requireLeadgenAdmin();
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.from("leadgen_users").update({ payroll_currency: currency }).eq("id", agentId);
