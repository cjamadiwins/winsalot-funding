...
import { closeOpportunityAction } from "./opportunities/[id]/actions";
...
// Admin equivalent of the agent dashboard's overdue-opportunities panel -
// same three quick actions, same shared Modal/CloseOpportunityPanel and
// reschedule/complete/close server actions (admin-scoped versions), so an
// admin can handle routine overdue follow-ups across every agent's
// opportunities without opening each one. Unlike an agent's own panel
// (implicitly "my opportunities"), every card also shows which agent the
// opportunity is assigned to.
export default function AdminOverdueOpportunitiesPanel({
  opportunities,
  followUps,
  agents,
}: {
  ...
  const reschedulingOpportunity =
    overdueOpportunities.find((o) => o.id === reschedulingOpportunityId) ?? null;
    ...
    <div className="rounded-2xl border-2 border-rose-300 bg-rose-50 p-4 sm:p-5">
      ...
      <ul className="mt-3 space-y-3">
        {overdueOpportunities.map((opportunity) => {
          ...
          return (
            <li key={opportunity.id} className="rounded-xl border border-rose-200 bg-[var(--crm-surface)] p-4">
              ...
              <div className="mt-1 text-[13px] text-slate-500">
                ...
                {opportunity.phone} · Assigned to {agent?.full_name || agent?.email || "Unassigned"}
              </div>
              ...
              <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                ...
                <button
                  ...
                  disabled={!followUp || isPending}
                  onClick={() =>
                    runAction(() => completeFollowUpAction(followUp!.id, opportunity.id))
                  }
        ...
        <Modal title="Called — Reschedule" onClose={() => setReschedulingOpportunityId(null)}>
          ...
          <form
            action={(formData) =>
              runAction(
                () =>
                  rescheduleFollowUpAction(reschedulingFollowUp.id, reschedulingOpportunity.id, formData),
