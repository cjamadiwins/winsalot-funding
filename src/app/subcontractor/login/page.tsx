...
export default async function SubcontractorLoginPage({
  searchParams,
}: {
  ...
  return (
    <div className="crm-theme flex min-h-screen items-center justify-center bg-[var(--color-bg)] px-4">
      <div className="w-full max-w-sm rounded-2xl border border-[var(--color-border)] bg-[var(--color-input-bg)] p-8 shadow-sm">
        <h1 className="font-heading text-xl font-bold text-[var(--color-ink-strong)]">Subcontractor Sign In</h1>
        ...
        <form action={subcontractorLoginAction} className="mt-6 space-y-4">
          <input type="hidden" name="redirectTo" value={params.redirectTo ?? "/subcontractor/dashboard"} />
        ...
        <a href="/subcontractor/forgot-password" className="mt-4 block text-center text-sm font-medium text-[var(--color-accent)]">
          Forgot password?
        </a>
      </div>
    </div>
