...
// Subcontractor twin of src/app/agent/set-password/SetPasswordClient.tsx -
// same invite/recovery-link verification logic (see that file's own
// header comment for the full rationale on why "a session merely exists"
// is never trusted as proof of a fresh link), just redirecting into the
// /subcontractor portal instead of /agent.
      ...
      <div className="w-full max-w-sm rounded-2xl border border-[var(--color-border)] bg-[var(--color-input-bg)] p-8 shadow-sm">
        ...
        {status === "invalid" && (
          <div className="mt-6 space-y-3">
            ...
            <a href="/subcontractor/login" className="text-sm font-semibold text-[var(--color-accent)]">
              Back to sign in
            </a>
          </div>
        )}
