import { requestSubcontractorPasswordResetAction } from "./actions";

export default async function SubcontractorForgotPasswordPage({
  searchParams,
}: {
  ...
  return (
    <div className="crm-theme flex min-h-screen items-center justify-center bg-[var(--color-bg)] px-4">
      <div className="w-full max-w-sm rounded-2xl border border-[var(--color-border)] bg-[var(--color-input-bg)] p-8 shadow-sm">
        <h1 className="font-heading text-xl font-bold text-[var(--color-ink-strong)]">Forgot Password</h1>
        ...
        {sent ? (
          ...
          <form action={requestSubcontractorPasswordResetAction} className="mt-6 space-y-4">
            <div>
        ...
        <a href="/subcontractor/login" className="mt-6 block text-center text-sm font-medium text-[var(--color-accent)]">
          Back to sign in
        </a>
      </div>
    </div>
