  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4" onClick={submitting ? undefined : onClose}>
    <div role="dialog" aria-modal="true" aria-label="Send Email" onClick={(e) => e.stopPropagation()} className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-2xl">
      ...
      {!kind ? <div className="mt-5 space-y-3">
        ...
        <button type="button" onClick={() => setKind("consultation")} className="w-full rounded-xl border border-slate-200 p-4 text-left transition hover:border-slate-400"><span className="block font-semibold text-slate-900">Consultation Invitation</span><span className="mt-1 block text-sm text-slate-500">Use the existing follow-up and consultation booking email.</span></button>
        ...
        {(["lead_generation", "business_financing"] as DetailedService[]).map((value) => <button key={value} type="button" onClick={() => setService(value)} className="w-full rounded-xl border border-slate-200 p-4 text-left font-semibold text-slate-900 transition hover:border-sky-400 hover:bg-sky-50">{DETAILED_SERVICE_LABELS[value]}{value === "lead_generation" && <span className="ml-2 text-sm font-medium text-sky-700">Approved price: {formatLeadGenerationPrice(leadGenerationPricing)}</span>}</button>)}
        ...
        {kind === "consultation" ? <><div className="text-xs font-medium text-slate-500">Service type: {OPPORTUNITY_TYPE_LABELS[opportunityType]}</div><label className="flex flex-col gap-1.5"><span className="text-[13px] font-semibold text-slate-600">Subject</span><input value={subject} onChange={(e) => setSubject(e.target.value)} disabled={sent} className={inputClass} /></label><label className="flex flex-col gap-1.5"><span className="text-[13px] font-semibold text-slate-600">Message</span><textarea value={message} onChange={(e) => setMessage(e.target.value)} disabled={sent} className={`${inputClass} min-h-[260px] resize-y font-mono text-[13px]`} /></label></> : detailedPreview && service ? <>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Email preview</p><p className="font-semibold text-slate-900">Subject: {detailedPreview.subject}</p><div className="mt-3 max-h-[360px] space-y-3 overflow-y-auto rounded-lg bg-white p-4 text-sm leading-6 text-slate-700">{detailedPreview.paragraphs.map((paragraph, index) => <div key={index}><p className="whitespace-pre-line">{paragraph}</p>{paragraph === "What Our Lead Generation Service Includes" && <ul className="mt-2 list-disc pl-5">{detailedPreview.bullets.map((item) => <li key={item}>{item}</li>)}</ul>}</div>)}<div className="pt-1 text-center"><a href={continueUrl} target="_blank" rel="noopener noreferrer" className="inline-block rounded-lg bg-sky-600 px-5 py-3 font-bold text-white">Continue with Winsalot Corp</a></div></div></div>
        </> : null}
        ...
        {kind === "detailed_service_pricing" && detailedPreview && service && <p role="note" className="rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-[13px] leading-5 text-amber-950"><strong>Before sending:</strong> Confirm the prospect has requested service/pricing information, verify the correct service is selected, and review the email preview. The standard Lead Generation price is <strong>$750/month</strong>. Do not offer or mention a pilot program unless approved by Admin.</p>}
