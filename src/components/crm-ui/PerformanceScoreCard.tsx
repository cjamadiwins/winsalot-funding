...
export default function PerformanceScoreCard({
  ...
  return (
    <section className={`rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-sky-50 p-8 shadow-sm sm:p-10 ${className}`}>
      <div className="flex flex-col items-center gap-10 sm:flex-row sm:items-stretch">
        <div className="flex shrink-0 items-center justify-center sm:border-r sm:border-slate-200 sm:pr-10">
          <PerformanceRing percentage={score} tier={tier} size={gaugeSize} strokeWidth={strokeWidth} segments={segments} />
        </div>

        <div className="min-w-0 flex-1 text-center sm:pl-4 sm:text-left">
          <h2 className="truncate text-[30px] font-bold text-slate-900">{agentName}</h2>
          <span
            className={`mt-2.5 inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-[14.5px] font-semibold ${style.badge}`}
          >
            <span className={`h-2.5 w-2.5 rounded-full ${style.bar}`} aria-hidden="true" />
            {band.label}
          </span>

          <p className="mt-5 text-[16.5px] font-semibold text-slate-700">{resultsLine}</p>
          <p className="mt-1.5 text-[14px] text-slate-500">{periodLabel}</p>

          <div className="mt-5 flex items-start gap-3.5 rounded-xl bg-sky-50 p-5 text-left">
            ...
            <div className="min-w-0">
              <div className="text-[15.5px] font-bold text-slate-900">{message.headline}</div>
              <div className="mt-1 text-[14px] text-slate-600">{message.description}</div>
            </div>
          </div>

          {reportHref && (
            <Link href={reportHref} className="mt-5 inline-block whitespace-nowrap text-[14.5px] font-semibold text-sky-600 hover:text-sky-700">
              View full report →
            </Link>
          )}
        </div>
